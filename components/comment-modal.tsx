...
import { addComment, getPostComments, getUserProfile, type Comment } from "@/lib/firebase/firestore"
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      if (result.xpGained > 0) {
        // Aqui você pode adicionar uma notificação de XP se quiser
        console.log(`[v0] XP gained from comment: ${result.xpGained}`)
      }
    } catch (error) {
      console.error("[v0] Error adding comment:", error)
      const errorMessage = error instanceof Error ? error.message : "Erro ao adicionar comentário. Tente novamente."
      alert(errorMessage)
    } finally {
      setLoading(false)
    }
